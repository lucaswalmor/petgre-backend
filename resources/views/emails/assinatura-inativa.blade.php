...
<head>
    ...
    <title>Assinatura em atraso</title>
</head>
                ...
                <table width="600" cellpadding="0" cellspacing="0" border="0" style="background-color:#ffffff; border-radius:12px; box-shadow:0 4px 6px rgba(0,0,0,0.1); overflow:hidden;">
                    <tr>
                        <td style="padding:40px;">
                            <h1 style="color:#dc2626; font-size:24px; margin:0 0 20px 0;">PetGre – Pagamento em atraso</h1>
                            ...
                            <p style="color:#4b5563; font-size:15px; line-height:1.6; margin:0 0 15px 0;">
                                Sua assinatura está em atraso (vencimento: {{ $vencimento }}). Valor: <strong>R$ {{ number_format($valor, 2, ',', '.') }}</strong>.
                            </p>
                            @if($linkFatura)
                                <p style="margin:0 0 15px 0;"><a href="{{ $linkFatura }}" style="color:#3b82f6;">Pagar fatura</a></p>
                            @endif
                            @if($pixCopiaCola)
                                <p style="color:#4b5563; font-size:14px; margin:0;">PIX Copia e Cola:</p>
                                <p style="word-break:break-all; font-size:12px; background:#f3f4f6; padding:10px; margin:5px 0 0 0;">{{ $pixCopiaCola }}</p>
                            @endif
                        </td>
                    </tr>
                </table>
